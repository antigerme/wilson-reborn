...
internamente de **SCRANTIC** (de *Screen Antics*) — daí o programa de
screensaver `SCRANTIC.SCR` (um `.exe` renomeado) e o ícone `SCRANTIC.ICO`.
...
- **`SCRANTIC.SCR`** — o **executável do screensaver** (`.scr` = `.exe` renomeado; embute o
  motor, a tabela de *walk* e os 23 efeitos sonoros). As imagens de fundo são recursos `.SCR`
  **dentro** do `RESOURCE.001`, não este arquivo.
...
## 5. Linhagem open-source (os projetos de referência)
...
4. **Licenças:** jc_reborn e JCOS são **GPLv3**; castaway/dgds-viewer têm licença
   própria (ver o `LICENSE` de cada upstream); o engine DGDS do ScummVM é **GPLv2+ (ScummVM)**.
