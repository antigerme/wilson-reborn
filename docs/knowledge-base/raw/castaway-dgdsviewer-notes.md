...
- **castaway** — `/home/user/wilson-reborn/repos/castaway` — a focused re-implementation of *Johnny Castaway* as native browser **ES modules** (`.mjs`), no build step. Served with `http-server`. Aim: "complete re-implementation of the Johnny Castaway Screen Saver ... using Javascript modules."
- **dgds-viewer** — `/home/user/wilson-reborn/repos/dgds-viewer` — a more general **React/Electron** resource browser/player for *multiple* Dynamix DGDS games. Built with webpack/babel, Electron shell, Express SSR dev server.
